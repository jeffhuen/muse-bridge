...
func TestPhase1RecheckRestoredCacheValidatesToolContent(t *testing.T) {
 for _,streaming:=range []bool{false,true} {t.Run(fmt.Sprintf("stream=%v",streaming),func(t *testing.T){
  ...
  if restored.GetToolArgs("call_restore")["key"]!="original" {t.Fatal("original arguments did not survive save/load")}
  ...
  replay:=func()(*upstream.PredictionRequest,error){
   ...
   raw,_:=json.Marshal([]any{map[string]any{"role":"user","content":"look up"},call,map[string]any{"type":"function_call_output","call_id":"call_restore","output":"result"}})
